export function rtfToText(rtf: string): string {
  ...
  while ((match = rtfRegex.exec(rtf)) !== null) {
      if (match[0] === "{") {
          ...
          output.splice(stack.pop()!, 0); // Use non-null assertion since stack is not empty here
      } else if (match[0][0] === "\\") {
          if (match[1] === "par" || match[1] === "line") {
              ...
              output.push(String.fromCharCode(parseInt(match[3]!, 16))); // Use non-null assertion for match[3]
          }
      } else {
          output.push(match[0]);
      }
